...
c-------------------------------------------------------------------
     ...
     1  ,nswt,lswnam,ldeflt,ifreq,nmin,nmax,icnt,iopn,iptr,itable,lcom)
      ...
      character*30 swnam(*)
      character*80 deflt(*)
      dimension lswnam(*),ldeflt(*),ifreq(*),nmin(*),nmax(*)
     1         ,icnt(*),iopn(*),iptr(*),itable(100,*)
        ...
        if(c.eq.'|') then
          ...
          if(nswt.gt.0) then
            ...
            if(ifrq.eq.0) then
              ...
              mss=mss+1
              do while (.TRUE.)
                if(mss.lt.mee.and.string(mss:mss).ne.']') then
                  ldef=ldef+1
   ...
   77         continue
      ...
      if(lswnam(1).eq.0.or.swnam(1).eq.' ') then
        if(nmax(1).gt.0) then
          ioplv=1+ioplv
          iopn(ioplv)=1
          icnt(1)=0
      ...
      do iarg=1,narg
        ...
        ip2=k
        klen=k+1
        if(klen.gt.llen) stop 'chekgl: command line too long'
        ln=ip2-ip1+1
   ...
   22   continue
        if(imatch.gt.1) then
          ...
          if(cline(ip1:ip1).eq.'-'.and.ip2.gt.1+ip1
     1      .and.ibreak.eq.0.and.
     1    ( (cline(ip1+1:ip1+1).ge.'a'.and.cline(ip1+1:ip1+1).le.'z')
     1            .or.(cline(ip1+1:ip1+1).ge.'A'.and.cline(ip1+1:ip1+1).le.'Z'))  ) then
            ibreak=1
            goto 67
          endif
   ...
   52 continue
     ...
     1                        .or.icnt(i).gt.nmax(i))) then
            ierr=4
            write(*,*) i,icnt(i),nmin(i),nmax(i)
          endif
        enddo
      endif
      if(ierr.ne.0) then
